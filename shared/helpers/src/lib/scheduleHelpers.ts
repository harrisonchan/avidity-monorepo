...
import { compareDuration, getStandardFormat, shuffleArray } from '@shared/utils';
...
function createTimeSlots(params: { timeBlocks: TimeBlock[]; scheduleStart?: Dayjs; scheduleEnd?: Dayjs }): TimeSlot[] {
  ...
  if (scheduleStart) {
    timeBlocks.unshift({
      start: scheduleStart.subtract(1, 'minute'),
      end: scheduleStart,
    });
  }
  if (scheduleEnd) {
...
export function createSchedule(params: { date: DateParam; goals: Goal[]; scheduleOptions?: ScheduleOptions }): GoalSchedule {
  ...
  const scheduleOptions: Required<ScheduleOptions> = {
    ...
    scheduleRespiteOptions: {
      respites: params.scheduleOptions?.scheduleRespiteOptions?.respites ?? [
        {
          start: date.set('hour', 12).set('minute', 0),
          end: date.set('hour', 13).set('minute', 0),
        },
      ], // defaults to lunch
      blockedRespites: params.scheduleOptions?.scheduleRespiteOptions?.blockedRespites ?? [
        {
          start: date.set('hour', 22).set('minute', 0),
          end: date.add(1, 'day').set('hour', 6).set('minute', 30),
        },
      ],
    ...
    defaultGoalDuration: params.scheduleOptions?.defaultGoalDuration ?? {
      minutes: 30,
    },
  ...
  let sortedGoalsWithoutTime = goalsWithoutTime.map((goal) => ({
    ...goal,
    duration: scheduleOptions.defaultGoalDuration!,
    ...
    goalsWithTime.forEach((goalOne, i) => {
      goalsWithTime.forEach((goalTwo, _i) => {
        ...
        if (
          hasTimeConflict(
            {
              start: dayjs(goalOneDateTime.start.dateTime),
              end: dayjs(goalOneDateTime.end.dateTime),
            },
            {
              start: dayjs(goalTwoDateTime.start.dateTime),
              end: dayjs(goalTwoDateTime.end.dateTime),
            },
            '[]'
          ) &&
          i !== _i
  ...
  let timeBlocks: TimeBlock[] = sortedGoalsWithTime
    .map((_g) => ({
      start: dayjs(_g.dateTimeData.start.dateTime),
      end: dayjs(_g.dateTimeData.end.dateTime),
    }))
          ...
          schedule.push(
            convertGoalToScheduledGoal({
              ...
              dateTimeData: {
                start: {
                  date: start.date,
                  dateTime: timeSlot.start.format(),
                  timeZone: '',
                },
                end: {
                  date: end.date,
                  dateTime: timeSlot.start.add(goalDuration, 'millisecond').format(),
                  timeZone: '',
                },
                status,
              },
  ...
  const disallowedCategoriesGoalsWithDuration: (Goal & {
  ...
  const disallowedCategoriesGoalsWithoutTime: (Goal & {
    duration: GoalDuration;
  })[] = [];
